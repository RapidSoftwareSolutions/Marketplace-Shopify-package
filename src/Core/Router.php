...
class Router
{
    ...
    public function setup()
    {
        ...
        $this->klein->respond('POST', INDEX_PATH.'api/Shopify/webhookEvent', function ($request, $response){
    ...
    private function setRoute($block)
    {
        ...
        if(
            ...
            in_array($this->custom[$block['name']]['method'], ['POST', 'GET', 'PUT', 'PUTGET', 'PATCH', 'DELETE'])
        ){
        ...
        $this->klein->respond('POST', $routePath, function()use($param, $blockName, $blockCustom, $method){
            ...
            if(isset($blockCustom['custom'])){
                ...
                if(isset($this->custom[$blockName]['wrap'])&&strlen($this->custom[$blockName]['wrap'])>0){
                    $sendBodyContent = $sendBody;
                    $sendBody = [];
                    $sendBody[$this->custom[$blockName]['wrap']] = $sendBodyContent;
                }
                $sendBody = json_encode($sendBody);
            }

            // Make request
            $result = $this->httpRequest($vendorUrl, $method, $accessToken, $sendBody);
            echo json_encode($result);
            exit(200);
        });
        return true;
    ...
    protected function httpRequest($url, $method, $accessToken, $sendBody)
    {
        ...
        try {
            ...
            if(strlen($accessToken)>1){
                $clientSetup['headers']['X-Shopify-Access-Token'] = $accessToken;
            }

            if($method == 'GET' || $method == 'PUTGET'){
